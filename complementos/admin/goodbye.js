...
module.exports = {
  nome: 'goodbye',
  desc: 'Configura mensajes de despedida',
  run: async (client, msg, args) => {
    try {
      ...
      if (action === 'on') {
        // Activar despedidas
        global.config.goodbye = text || 'Adiós @user, esperamos verte pronto!';
        // Guardar en archivo
        const configPath = path.join(__dirname, '../../configuracion/config.json');
        fs.writeJsonSync(configPath, global.config, { spaces: 2 });
        try {
          await chat.sendMessage('Mensajes de despedida activados.');
        } catch (e) {
          console.error('Error enviando mensaje:', e);
        }
      } else if (action === 'off') {
        // Desactivar
        global.config.goodbye = null;
        // Guardar en archivo
        const configPath = path.join(__dirname, '../../configuracion/config.json');
        fs.writeJsonSync(configPath, global.config, { spaces: 2 });
        try {
          await chat.sendMessage('Mensajes de despedida desactivados.');
        } catch (e) {
          console.error('Error enviando mensaje:', e);
        }
      } else {
        try {
          await chat.sendMessage('Uso: .goodbye on/off [mensaje]');
        } catch (e) {
          console.error('Error enviando mensaje:', e);
        }
      }
      ...
      console.error(error);
      try {
        await client.sendMessage(msg.from, { text: 'Error al configurar despedidas.' });
      } catch (e) {
